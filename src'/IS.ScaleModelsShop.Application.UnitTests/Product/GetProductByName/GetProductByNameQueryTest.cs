...
namespace IS.ScaleModelsShop.Application.UnitTests.Product.GetProductByName
{
    public class GetProductByNameQueryTest
    {
        ...
        {
            _productRepositoryMock.Setup(x => x.GetEntityByNameAsync(It.IsAny<Expression<Func<Domain.Entities.Product, bool>>>()))
                .ReturnsAsync( new Domain.Entities.Product());

            await _handler.Handle(new GetProductByNameQuery(), CancellationToken.None);

            _productRepositoryMock.Verify(x => x.GetEntityByNameAsync(It.IsAny<Expression<Func<Domain.Entities.Product, bool>>>()), Times.Once);
        }
